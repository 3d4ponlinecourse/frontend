  ...
  return (
    <div className="flex items-start w-full px-8 m-5 gap-8">
      <div className="justify-center border border-black grid gap-y-16 rounded-3xl py-8 w-4/12">
        ...
        <div className="flex-col justify-start">
          <div className="grid gap-y-8 text-2xl  flex-col">
            ...
            <button type="button" onClick={() => setComponent(2)} className="flex justify-start font-semibold">
              Edit Profile
            </button>
          </div>
        </div>
      ...
      <div className="w-full">
        {component === 1 ? <CurrentCourse user={user} /> : null}
        {component === 2 ? <Editprofile /> : null}
      </div>
    </div>
  )
}
